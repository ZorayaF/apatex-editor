...
## **Construcción del Ejecutable (Build)**

El proceso de empaquetado y distribución actualmente está configurado exclusivamente para entornos **Windows** (`.exe`).

Para compilar y empaquetar la aplicación:

1. Ubicarse en la raíz del proyecto.
2. Ejecutar el script de distribución:

```
npm run dist
```

Una vez finalizado el proceso, los instaladores y el archivo binario (`.exe`) se encontrarán disponibles en el directorio:
